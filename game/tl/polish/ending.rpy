# TODO: Translation updated at 2014-06-19 20:21

# game/ending.rpy:7
translate polish badend_3470d86e:

    # centered "И захватили машины этот мир." with dissolve
    centered "Maszyny przejęły kontrolę nad światem." with dissolve

# game/ending.rpy:8
translate polish badend_85dbbeef:

    # centered "И было всё плохо. " with dissolve
    centered "Ludzi spotkało niewyobrażalne cierpienie." with dissolve

# game/ending.rpy:9
translate polish badend_af58bbe5:

    # centered "Ибо для победы человечества не хватило всего одних рабочих рук... " with dissolve
    centered "By zapewnić ludzkości zwycięstwo, tylko jedna para rąk była jeszcze potrzebna..." with dissolve

# game/ending.rpy:10
translate polish badend_b7051b2f:

    # centered "Твоих рук! " with dissolve
    centered "Twoja!" with dissolve

# game/ending.rpy:11
translate polish badend_4979658d:

    # centered "{size=36}{b}ПЛОХАЯ КОНЦОВКА{/b}{/size}" with wipeleft
    centered "{size=36}{b}ZŁE ZAKOŃCZENIE{/b}{/size}" with wipeleft

# game/ending.rpy:12
translate polish badend_007ef76e:

    # centered "( P.S. Не забудьте найти другие концовки! )" with irisout
    centered "( P.S. Nie zapomnij znaleźć innych zakończeń! )" with irisout

# game/ending.rpy:20
translate polish goodend_f84785f8:

    # centered "Вместе мы сделали ещё много-много релизов."
    centered "Razem jesteśmy w stanie wydawać więcej tomów mang."

# game/ending.rpy:21
translate polish goodend_ff8f1f6f:

    # centered "И были все счастливы!"
    centered "I wszyscy byli szczęśliwi!"

# game/ending.rpy:22
translate polish goodend_5e6fc80b:

    # centered "{size=36}{b}ХОРОШАЯ КОНЦОВКА{/b}{/size}" with wipeleft
    centered "{size=36}{b}DOBRE ZAKOŃCZENIE{/b}{/size}" with wipeleft

# game/ending.rpy:30
translate polish titles_37e31ff4:

    # centered "Над игрой работали:" with dissolve
    centered " Autorzy:" with dissolve

# game/ending.rpy:39
translate polish titles_de93b2c7:

    # centered "Сюжет, графика, музыка,\n идея и реализация:\n {b}DeXPeriX{/b} a.k.a. Храбров Дмитрий\n {a=http://dexperix.net}http://dexperix.net{/a}"
    centered "Historia, grafiki, muzyka,\n pomysł i realizacja:\n {b}DeXPeriX{/b} a.k.a. Hrabrov Dmitry\n {a=http://dexperix.net}http://dexperix.net{/a}"

# game/ending.rpy:53
translate polish titles_e49fc160:

    # centered "Перевод на английский:\n {b}Mari Prianaja{/b},\n {b}DeXPeriX{/b} a.k.a. Храбров Дмитрий\n\n Улучшения английского текста:\n {b}Mangles{/b} a.k.a. Richard Hogan,\n Warren '{b}Wazza{/b}' Brent"
    centered "Angielskie tłumaczenie:\n {b}Mari Prianaja{/b},\n {b}DeXPeriX{/b} a.k.a. Hrabrov Dmitry\n\nOgólna korekta i kontrola jakości:\n {b}Mangles{/b} a.k.a. Richard Hogan,\n Warren '{b}Wazza{/b}' Brent"


# game/ending.rpy:61
translate polish titles_753c51bc:

    # centered "Вычитка английского текста:\n bluecasuality@gmail.com (День 1)\n {b}EeuorT{/b} a.k.a. Robert Nesbitt (День 2, 7)\n {b}PintSizedPurple{/b} a.k.a. Will Williams (3, 4)\n {b}Mangles{/b} a.k.a. Richard Hogan (День 5)\n Warren '{b}Wazza{/b}' Brent (День 6)" with dissolve
    centered "Korekta angielskiego tekstu:\n bluecasuality@gmail.com (Dzień 1)\n {b}EeuorT{/b} a.k.a. Robert Nesbitt (Dzień 2, 7)\n {b}PintSizedPurple{/b} a.k.a. Will Williams (3, 4)\n{b}Mangles{/b} a.k.a. Richard Hogan (Dzień 5)\nWarren '{b}Wazza{/b}' Brent (Dzień 6)" with dissolve


# game/ending.rpy:63
translate polish titles_130856ff:

    # centered "Вычитка русского текста:\n {b}Selene=){/b} a.k.a. Березовская Любава,\n {b}DeXPeriX{/b} a.k.a. Храбров Дмитрий,\n {b}Mari Prianaja{/b}"
    centered "Korekta rosyjskiego tekstu:\n {b}Selene=){/b} a.k.a. Berezovskaya Lyubava,\n {b}DeXPeriX{/b} a.k.a. Hrabrov Dmitry,\n {b}Mari Prianaja{/b}"
    
# game/ending.rpy:92
translate polish titles_90a1d8eb:

    # centered "Перевод на другие языки:\n\n {image=items/flags/pl.jpg} Польский: damianklu a.k.a. Damian K."
    centered "Tłumaczenia na inne języki:\n\n {image=items/flags/pl.jpg} Polski: damianklu a.k.a. Damian K."

# game/ending.rpy:65
translate polish titles_4aa106c5:

    # centered "Вспомогательные элементы сюжета:\n {b}Lybares{/b} a.k.a. Губарь Любовь,\n {b}Malvina{/b} a.k.a. Скрябина Татьяна,\n {b}lapschedui{/b} a.k.a. Погорельченко Людмила  "
    centered "Pomoc przy fabule:\n {b}Lybares{/b} a.k.a. Gubar Lubov,\n {b}Malvina{/b} a.k.a. Skryabina Tatyana,\n {b}lapschedui{/b} a.k.a. Pogorelchenko Ludmila  "

# game/ending.rpy:123
translate polish titles_e51d6f6e:

    # centered "Изображения для сканирования и клина:\n {b}eng. Zubr{/b} a.k.a. Бурим Алексей\n \n Бэта-тестеры:\n {b}Ulilla{/b} a.k.a. Байкова Юлия,\n {b}Bismut{/b} a.k.a. Жаворонков Дмитрий"
    centered "Pomoc przy grafikach:\n {b}eng. Zubr{/b} a.k.a. Burim Alex\n \n Beta-testerzy:\n {b}Ulilla{/b} a.k.a. Baykova Yulia,\n {b}Bismut{/b} a.k.a. Zhavoronkov Dmitry"


# game/ending.rpy:90
translate polish titles_b7646793:

    # centered "Использованные средства:\n - Движок: {a=http://renpy.org}RenPy{/a} от PyTom ({a=http://www.renpy.org/dl/lgpl/}LGPL{/a})\n - Звуки с сайта {a=http://freesound.org}FreeSound.org{/a}\n - Графика: {a=http://www.comipo.com/en/}Manga Maker ComiPo!{/a}\n - Дополнительная графика: {a=http://gimp.org}GIMP{/a}\n - Музыка: {a=http://magix.com/}Magix Music Maker 2014{/a}"
    centered "Użyte zasoby:\n - Silnik: {a=http://renpy.org}RenPy{/a} od PyTom ({a=http://www.renpy.org/dl/lgpl/}LGPL{/a})\n - Dźwięki z {a=http://freesound.org}FreeSound.org{/a}\n - Grafiki: {a=http://www.comipo.com/en/}Manga Maker ComiPo!{/a}\n - Dodatkowe grafiki: {a=http://gimp.org}GIMP{/a}\n - Muzyka: {a=http://magix.com/}Magix Music Maker 2014{/a}"

# game/ending.rpy:99
translate polish titles_572dfca6:

    # centered "Сторонние 3D-модели:\n - Frilly Dress от {a=http://chibi-baka-san.deviantart.com/}Chibi-Baka-San{/a}\n - Long Nightie из набора\n \"B Cup Clothes\" от {a=http://ina-haru.deviantart.com/}Ina-Haru{/a}\n - Maid Outfit от {a=http://www.comee.jp/userinfo.php?userid=457}Hekemoko{/a}\n - Branch of tree от {a=http://www.mangasozaibox.comee.jp/users/user_info/id:13}Sion 42{/a}"
    centered "Modele 3D osób trzecich:\n - Frilly Dress od {a=http://chibi-baka-san.deviantart.com/}Chibi-Baka-San{/a}\n - Long Nightie z zestawu \n \"B Cup Clothes\" od {a=http://ina-haru.deviantart.com/}Ina-Haru{/a}\n - Maid Outfit od {a=http://www.comee.jp/userinfo.php?userid=457}Hekemoko{/a}\n - Branch of tree od {a=http://www.mangasozaibox.comee.jp/users/user_info/id:13}Sion 42{/a}"

# game/ending.rpy:111
translate polish titles_ffcdbd54:

    # centered "Больше информации можно найти на сайте:\n{a=http://OneMangaDay.dexp.in}http://OneMangaDay.dexp.in{/a}\n \n Или на сайте автора:\n {a=http://dexperix.net}http://dexperix.net{/a}" with dissolve
    centered "Więcej informacji można znaleźć na:\n{a=http://OneMangaDay.dexp.in}http://OneMangaDay.dexp.in{/a}\n \n Lub na stronie autora:\n {a=http://dexperix.net}http://dexperix.net{/a}" with dissolve

# game/ending.rpy:143
translate polish titles_5b0beb4e:

    # centered "Больше информации можно найти на сайте:\n{a=http://OneMangaDay.dexp.in}http://OneMangaDay.dexp.in{/a}\n \n Или на сайте автора:\n {a=http://dexp.in}http://dexp.in{/a}" with dissolve
    centered "Więcej informacji można znaleźć na:\n{a=http://OneMangaDay.dexp.in}http://OneMangaDay.dexp.in{/a}\n \n Lub na stronie autora:\n {a=http://dexp.in}http://dexp.in{/a}" with dissolve

# game/ending.rpy:159
translate polish titles_5f70d195:

    # centered "Отдельное спасибо проголосовавшим за игру в {a=http://steamcommunity.com/sharedfiles/filedetails/?id=372656709}Steam Greenlight{/a}.\n И площадке {a=http://store.steampowered.com/app/365070}Steam{/a} в общем за возможность опубликовать игру.\n \n Так же огромное спасибо всем бэта-тестрам финальной версии. Без вас игра не получилась бы настолько хорошей!"
    centered "Specjalne podziękowania dla wszystkich, którzy\n zagłosowali na grę na {a=http://steamcommunity.com/sharedfiles/filedetails/?id=372656709}Steam Greenlight{/a},\n beta-testerom Steam i {a=http://store.steampowered.com/app/365070}Steam{/a} za umożliwienie publikacji."

# game/ending.rpy:118
translate polish titles_f2b2b59f:

    # centered "Спасибо, что были с нами!" with dissolve
    centered "Dzięki za bycie z nami!" with dissolve

# game/ending.rpy:123
translate polish titles_09d06917:

    # lin "Увидимся в следующей части!"
    lin "Do zobaczenia w kolejnej grze!"

# game/ending.rpy:128
translate polish titles_fed84019:

    # drakoha "Эй! А я только с солью вернулась..."
    drakoha "Hej! Wróciłam ze sklepu! Przyniosłam sól..."


# game/ending.rpy:147
translate polish titles_af07b759:

    # centered "\nРоли озвучивали (команда {a=http://xdub.ru/}xdub.ru{/a}):\n {b}Izanami{/b} a.k.a. Хабарова Анастасия - Лайби, Кэрри, Пряня, бабушка\n {b}Elrid{/b} a.k.a. Палагута Александр - Дэкс, Бог-облако, работа со звуком\n {b}Sharon{/b} a.k.a. Игнатенко Александр - Андрей, авторская речь\n {b}Kimamire{/b} a.k.a. Крашник Оля - Лап, Юлилла, Мирей, Джесс\n {b}MIX-MAN{/b} a.k.a. Аввакумов Максим - Лёха\n {b}Kobayashi{/b} a.k.a. Ставоржинская Яна - Мальвина, Лин, Азуки, Дракоша, Янушка, незнакомка\n"
    centered "\nRosyjski głos ({a=http://xdub.ru/}xdub.ru{/a}):\n {b}Izanami{/b} a.k.a. Khabarova Anastasia - Laiby, Carry, Spicy, granny\n {b}Elrid{/b} a.k.a. Palaguta Alexander - Dex, God-cloud, working with sound\n {b}Sharon{/b} a.k.a. Ignatenko Alexandr - Andre, the author's speech\n {b}Kimamire{/b} a.k.a. Krashnik Olya - Lap, Yulilla, Mirei, Jess\n {b}MIX-MAN{/b} a.k.a. Avvakumov Maxim - Alex\n {b}Kobayashi{/b} a.k.a. Stavorjinskaya Yana - Malia, Lin, Azuki, Dragonha, Yana, stranger\n"

